...
try:
    ...
    for root, dirs, files in os.walk(image_dir):
        for file in files:
            if fnmatch.fnmatch(os.path.splitext(file)[0], filename):
                if file.endswith("jpeg") or file.endswith("jpg") or file.endswith("png"):
                    #paths = os.path.join(root, file)                    
                    
                    para = get_string("./images/"+ file).lower()  # get_string() we are extractiion string from the receipt and they type is 0 which is default 
                    #print(para)
                    checkme = main_function(para) #own defined function

                    if(checkme == 0 ):
                        para = get_string("./images/"+ file, types=1).lower()  # get_string() type is 1 means to reduce the size of image by 80% height and width
                        """print("----------------------------------------getting new string with type 1")
                        print(para)"""                        
                        checkme = main_function(para)

                        if checkme == 0:
                            para = get_string("./images/"+ file, types=2).lower() # get_string() type is 2 means to enhance the sharpness by 5.0 points
                            """print("----------------------------------------getting enhance string with type 2")
                            print(para)"""
                            checkme = main_function(para)

                            if checkme == 0:
                                para = get_string("./images/"+ file, types=3).lower() # get_string() type is 3 means to enhance the sharpness by 3.0 points
